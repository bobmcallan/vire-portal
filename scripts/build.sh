...
build_binaries() {
    ...
    if [[ "$BUILD_TARGET" == "all" || "$BUILD_TARGET" == "portal" ]]; then
        [ -f "$CONFIG_DIR/vire-portal.toml" ] && [ ! -f "$BIN_DIR/vire-portal.toml" ] && cp "$CONFIG_DIR/vire-portal.toml" "$BIN_DIR/" || true
        rm -rf "$BIN_DIR/pages"
    ...
    if [[ "$BUILD_TARGET" == "all" || "$BUILD_TARGET" == "mcp" ]]; then
        [ -f "$CONFIG_DIR/vire-mcp.toml" ] && [ ! -f "$BIN_DIR/vire-mcp.toml" ] && cp "$CONFIG_DIR/vire-mcp.toml" "$BIN_DIR/" || true
    fi
